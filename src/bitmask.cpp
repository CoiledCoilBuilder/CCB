...
void Bitmask::add_bitmask(const char *name) {

  // Check to see that the bitmask name is legit
  int n = strlen(name);
  for (int i = 0; i < n; i++)
    if (!isalnum(name[i]) && name[i] != '_')
      error->one(FLERR, "bitmask: names must be alphanumeric or underscore characters");

  int imask = find(name);

  if (imask < 0) {
    if (nmask == MAX_BITMASK) error->all("bitmask: Too many bitmasks");
    imask = find_unused();
    int n = strlen(name) + 1;
...
void Bitmask::delete_bitmask(const char *name) {

  if (strcmp(name,"all") == 0)
    error->one(FLERR, "bitmask: can't delete \"all\" bitmask");
